...
This module loads the observed FRB catalog, models survey selection effects,
generates isotropic mock ensembles, measures angular clustering statistics,
estimates observational uncertainties, performs covariance-aware inference,
and exports diagnostic plots and reports.
"""
...
def apply_analysis_config(
    ...
    SUMMARY_CSV = f"summary_{RUN_TAG}.csv"
    DIAGNOSTICS_XLSX = f"diagnostics_{RUN_TAG}.xlsx"
    REPORT_MD = f"report_{RUN_TAG}.md"
    RESULTS_PKL = f"results_{RUN_TAG}.pkl"
...
print("\n==================================================")
print("FRB ISOTROPY PIPELINE CONFIGURATION")
print("==================================================")

print(f"USE_GAL_MASK : {USE_GAL_MASK}")
print(f"USE_SEL_FUNC : {USE_SEL_FUNC}")

print(f"GAL_CUT      : {GAL_CUT} deg")

print(f"BIN_SIZE     : {BIN_SIZE} deg")
print(f"N_BINS       : {N_BINS}")

print(f"NSIDE_SF     : {NSIDE_SF}")
...
class SelectionFunctionValidator:
    ...
    def plot_sf(
        ...
        plt.savefig(
            f"{save_prefix}_{RUN_TAG}_equatorial.png",
            dpi=150,
            bbox_inches="tight",
        )
        ...
        plt.savefig(
            f"{save_prefix}_{RUN_TAG}_galactic.png",
            dpi=150,
            bbox_inches="tight",
        )
...
def compute_statistics(
    ...
    pd.DataFrame(
        ...
        f"covariance_eigenvalues_{RUN_TAG}.csv",
        index=False,
    )
    ...
    plt.savefig(
        f"covariance_eigenspectrum_{RUN_TAG}.png",
        dpi=150,
        bbox_inches="tight",
    )
...
def run_jackknife_errors(
    ...
    plot_covariance_correlation_matrix(
        ...
        output=(
            f"JK_correlation_matrix_{RUN_TAG}"
        ),
    )
...
def run_bootstrap_errors(
    ...
    plot_covariance_correlation_matrix(
        ...
        output=(
            f"Bootstrap_correlation_matrix_{RUN_TAG}.png"
        ),
    )
...
def plot_results_with_jackknife(
    ...
    if output_2pacf is None:
        ...
        output_2pacf = (
            f"2pacf_jackknife_{RUN_TAG}.png"
        )

    if output_abs is None:

        output_abs = (
            f"absolute_anisotropy_jackknife_{RUN_TAG}.png"
        )
...
def plot_results_with_bootstrap(
    ...
    if output_2pacf is None:
        ...
        output_2pacf = (
            f"2pacf_bootstrap_{RUN_TAG}.png"
        )

    if output_abs is None:

        output_abs = (
            f"absolute_anisotropy_bootstrap_{RUN_TAG}.png"
        )
...
def plot_results_jk_vs_bootstrap(
    ...
    if output_2pacf is None:
        ...
        output_2pacf = (
            f"2pacf_jk_vs_bootstrap_{RUN_TAG}.png"
        )

    if output_abs is None:

        output_abs = (
            f"absolute_anisotropy_jk_vs_bootstrap_{RUN_TAG}.png"
        )
...
def save_report(
    ...
    if output_path is None:
        output_path = f"report_{RUN_TAG}.md"
...
def main(
    ...
    if USE_SEL_FUNC:
        ...
        validator.plot_sf(
            save_prefix=f"sf_validation_{RUN_TAG}"
        )
    ...
    plot_results_with_jackknife(
        ...
        all_abs_h0,
        jackknife,
        output_2pacf=f"2pacf_jackknife_{RUN_TAG}.png",
        output_abs=f"abs_jackknife_{RUN_TAG}.png",
    )
    ...
    plot_results_with_bootstrap(
        ...
        all_abs_h0,
        bootstrap,
        output_2pacf=f"2pacf_bootstrap_{RUN_TAG}.png",
        output_abs=f"abs_bootstrap_{RUN_TAG}.png",
    )
    ...
    plot_results_jk_vs_bootstrap(
        ...
        jackknife,
        bootstrap,
        output_2pacf=f"2pacf_jk_vs_bootstrap_{RUN_TAG}.png",
        output_abs=f"abs_jk_vs_bootstrap_{RUN_TAG}.png",
    )
